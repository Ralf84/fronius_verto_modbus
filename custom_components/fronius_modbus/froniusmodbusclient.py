...
class FroniusModbusClient(ExtModbusClient):
    ...
    async def read_inverter_data(self):
        ...
        try:
            ...
            TmpCab_raw = self._client.convert_from_registers(regs[3:4], data_type = self._client.DATATYPE.INT16)
            # Scale Factor für alle Temperaturen (Register 38)
            Tmp_SF = self._client.convert_from_registers(regs[7:8], data_type = self._client.DATATYPE.INT16)
